...
export class GeneratorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.item.fields.push(
      new Field("first", Types.STRING, "First Name", "", InputTypes.INPUT)
    );
    this.item.fields.push(
    ...
    this.item.fields.push(
      new Field("birthdate", Types.DATE, "Borthday", "", InputTypes.DATE)
    );
    this.item.fields.push(
  ...
  generateImportCode(obj: ItemObject): string {
    ...
    import { ${obj.name} } from '../${obj.name.toLowerCase()}';
    `;
  }
  generateComponentCode(obj: ItemObject, linebreak: string): string {
    ...
    const fg = [];
    obj.fields.forEach(f => {
      fg.push(` '${f.name}': new FormControl('')`);
    });
...
<div class="ui-grid-col-4"></div>
  ...
  generateHTMLCodeRow(f: Field): string {
    let result = "";
    let line = `<div class="ui-grid-row">
  ...
  getInputType(f: Field): string {
    ...
    switch (ty) {
      case InputTypes.INPUT: {
        return `<input pInputText type="text" formControlName="${f.name}" placeholder="Required"/>`;
        break;
      }
      case InputTypes.PASSWORD: {
        return `<input pInputText type="password" formControlName="${f.name}" placeholder="Required"/>`;
        break;
      }
